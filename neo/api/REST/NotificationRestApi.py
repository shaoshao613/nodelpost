                            ...
                            <pre>
{
    ...
    @app.route('%s/notifications/blocks/<string:contract_hash>' % API_URL_PREFIX, methods=['GET'])
    @cors_header
    def get_by_contracts(self, request, contract_hash):
        request.setHeader('Content-Type', 'application/json')
        try:
            ...
            return self.format_message(
                "Could not get notifications for contract hash %s because %s" % (contract_hash, e))
        return self.format_notifications(request, notifications)
